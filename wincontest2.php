...
$getwinners1="select * from winners2 order by createdAt desc";
if(isset($_POST['shuffle-2'])) {
$sql4="select * from ticket_info1 order by RAND() LIMIT 1";
$result3 = mysqli_query( $data,$sql4 );
while($row = mysqli_fetch_array($result3)) {
    $sql5="Insert into winners2 (ticket_id,price,quantity,name,email,address,city) values ('".$row["ticket_id"]."','".$row["price"]."','".$row["quantity"]."','".$row["name"]."','".$row["email"]."','".$row["address"]."','".$row["city"]."')";
    mysqli_query($data,$sql5);
    $sql6="truncate table ticket_info1";
    mysqli_query( $data,$sql6 );
...
<body>
    <div class="main">
        ...
        <form method="post" >
        <input type="submit" value="Shuffle-2" name="shuffle-2" class="btn">
         </form>
         </div>
        <table>
        <tr id="header">
        ...
        <tbody>
        <?php
                 $result4 = mysqli_query( $data,$getwinners1 );
